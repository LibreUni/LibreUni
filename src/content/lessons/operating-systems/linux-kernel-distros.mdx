...
{/* Source tracking: Linux kernel internals, CFS scheduler design, cgroups, namespaces, and distribution models synthesized from "Linux Kernel Development" (Love, 3rd Ed., Chapters 3-4), "The Linux Programming Interface" (Kerrisk, 1st Ed., Chapters 38, 53), and Wikipedia: Cgroups / Namespaces (CC-BY-SA 4.0). */}
...
<Quiz client:load
  ...
  question="Which Linux feature physically restricts CPU and Memory usage for a group of processes, and which feature isolates their view of system resources?"
  options={[
    "Namespaces restrict usage; Cgroups isolate the view.",
    "Cgroups restrict usage; Namespaces isolate the view.",
    "The CFS Scheduler restricts usage; VFS isolates the view.",
    "eBPF restricts usage; the VFS isolates the view."
  ]}
  correctIndex={1}
  explanation="Control Groups (Cgroups) enforce strict physical limitations on hardware resource consumption (such as RAM, CPU, and Disk I/O). Namespaces isolate what a process can see (e.g., process lists, network adapters, and file paths)."
/>
...
*   [Control groups](https://en.wikipedia.org/wiki/Cgroups) (CC-BY-SA 4.0)
*   [Linux namespaces](https://en.wikipedia.org/wiki/Linux_namespaces) (CC-BY-SA 4.0)
